...
At the Kickoff the team will cut to the core of this Vision, validating it and revealing what the project is really about and reaching team consensus on the what characteristics the initial value testable MVP will have and what jobs of work need prioritizing in order to make that delivery.
...
## Using Kanban (not Scrum) {#chapter01-kanban}

A core team meeting is now used to plan out on the Kanban board the actual jobs of work needed so that the team can build the prototype build (MVP 1) as quickly and directly as possible and have everything ready for the Team Kickoff.

Now, it's important to keep a Zen approach here. Instead of Just In Time and Agile being about super-exploitation, we want to exploit it, so that we optimize our productivity but are never overloaded. That means Keeping it Simple!

So the first thing we do with the Kanban board is to brainstorm about which jobs of work comply with the guideline of only implementing the minimum necessary to build the prototype as quickly as possible.
